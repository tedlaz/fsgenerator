...
from fsgenerator.type_mapping import id_python_type


def generate(
    entities: list[EntityDef], env: Environment, config: AppConfig
) -> list[tuple[str, str]]:
    files = []
    ...
    tenant_class_name = config.tenant.capitalize() if config.tenant else None
    files.append(
        (
            "main.py",
